...
| リポジトリ         | 役割                                                                     |
|---------------|------------------------------------------------------------------------|
| project       | PdMが管理するEpic issueや仕様書、他チーム向けのガイドラインなどを管理します。                          |
| project_api   | バックエンドのコードやバックエンドエンジニアのタスクを管理するissue、バックエンド領域における開発者向けのドキュメントなどを管理します。 |
| project_front | project_apiと同様                                                         |
...
プロジェクトに関するドキュメントが複数のリポジトリに分散するためにドキュメントの検索性や一覧性が低下しており、情報を探すのが難しい問題がありました。他方で、私たち自身も、情報を残す場所をどこにするかで迷うことが多かったです。

### After

プロジェクトに関する情報が1つのリポジトリに集約されたため、ドキュメントの探しやすさが向上しました。
...
共通のビルドジョブやデプロイプロセスの変更箇所が一箇所になり管理コストが削減された。
...
monorepoになり1コミットで複数のアプリケーションを変更できるようになったので、自分の変更によって他方のアプリケーションにも変更が必要な場面で、今まで触ってこなかった他方のコードも一緒に変更してしまおうと思うメンバーが出てきました。このように、monorepoにすることでメンバーのメンタルモデルが変化し、他のアプリケーションを開発する方法を学ぼうとしたり、プルリクエストを見ようとするメンバーが増えました。この変化によって、私はチーム内の分断が解消されたと感じています。

# 僕たちのMonorepo 1.0

Monorepoといっても、いろいろな形態があります。今回は、僕たちのMonorepo 1.0の方針を紹介します。
...
.github
apps
├ backend # 元project_apiリポジトリ
└ web     # 元project_frontリポジトリ
docs
```
